		...
		const char *icon, int allow_duplicate, result_cb_t result_cb, void *data)
{
	...
	body = g_variant_new("(i&s&si&s&si)", getpid(), appid, name, type, uri, icon, allow_duplicate);
		...
		const char *icon, double period, int allow_duplicate, result_cb_t result_cb, void *data)
{
	...
	body = g_variant_new("(i&s&si&s&sdi)", getpid(), widget_id, name, size, NULL, icon, period, allow_duplicate);
